 ...
 * @author      ALIENTEK
 * @brief       binary_semaphore_operation example
...
void task1(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        switch (key)
        {
            case KEY0_PRES:
            {
                xSemaphoreGive(BinarySemaphore);    /* release a binary semaphore */
                break;
            }
            default:
            {
...
void task2(void *pvParameters)
{
    ...
    while (1)
    {
        xSemaphoreTake(BinarySemaphore, portMAX_DELAY);             /* get a binary semaphore */
