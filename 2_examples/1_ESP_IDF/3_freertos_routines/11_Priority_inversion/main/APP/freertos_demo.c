 ...
 * @author      ALIENTEK
 * @brief       priority_inversion example
...
SemaphoreHandle_t Semaphore;                /* counter semaphore handle */
...
void task1(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        xSemaphoreTake(Semaphore, portMAX_DELAY);       /* get a counting semaphore */
        printf("task1 has taked semaphore\r\n");
        printf("task1 running\r\n");
        printf("task1 give semaphore\r\n");
        xSemaphoreGive(Semaphore);                      /* release a counting semaphore */
        vTaskDelay(100);
    }
}
